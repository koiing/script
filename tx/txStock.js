...
async function taskOldIDCheck() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task_daily.fcgi?action=home&type=routine&actid=1101&_=${rndtime}&openid=${signheaderVal}`
    );
          ...
          if (safeGet(data)) {
            ...
            switch (code) {
              case "0":
                ...
                const taskSetDescList = txTaskList.map(
                  (item) => item.reward_desc
                );
...
async function taskIDCheck() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task_daily.fcgi?action=home&type=routine&actid=1111&_=${rndtime}&openid=${signheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function wxtaskOldIDCheck() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task_daily.fcgi?t=${rndtime}`,
      `action=home&type=routine&actid=1100`
    );
...
async function wxtaskIDCheck() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task_daily.fcgi?t=${rndtime}`,
      `action=home&type=routine&actid=1110`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runAppTask(id, tid, ticket) {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1111&tid=${tid}&id=${id}&task_ticket=${ticket}&_appName=ios${taskheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runAppOldTask(id, tid, ticket) {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskdone&channel=1&actid=1101&tid=${tid}&id=${id}&task_ticket=${ticket}&_appName=ios${taskheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runWXTask(id, tid, ticket) {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      `_h5ver=2.0.1&actid=1110&tid=${tid}&id=${id}&task_ticket=${ticket}&action=taskdone`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runWXOldTask(id, tid, ticket) {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?t=${rndtime}`,
      `_h5ver=2.0.1&actid=1100&tid=${tid}&id=${id}&task_ticket=${ticket}&action=taskdone`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function apptaskticket() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskticket&channel=1&actid=1111&_rndtime=${rndtime}&_appName=ios${taskheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function appoldtaskticket() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?action=taskticket&channel=1&actid=1101&_rndtime=${rndtime}&_appName=ios${taskheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function WXtaskticket() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      ...
      `_h5ver=2.0.1&actid=1110&action=taskticket`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function WXoldtaskticket() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity_task.fcgi?`,
      `_h5ver=2.0.1&actid=1100&action=taskticket`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function getShareCode1() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?channel=1&action=query_share_code&share_type=task_51_1111&_=${rndtime}&openid=${signheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function getShareCode2() {
  return new Promise((resolve) => {
    const options = TaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?channel=1&action=query_share_code&share_type=news_share&_=${rndtime}&openid=${signheaderVal}`
    );
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function getWXShareCode1() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?`,
      `_h5ver=2.0.1&action=query_share_code&share_type=task_51_1110`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function getWXShareCode2() {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?`,
      `_h5ver=2.0.1&action=query_share_code&share_type=news_share`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runShareTask1(tasksharecode) {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?`,
      `_h5ver=2.0.1&action=share_code_info&share_type=task_51_1111&share_code=${tasksharecode}`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
async function runShareTask2(tasksharecode) {
  return new Promise((resolve) => {
    const options = wxTaskOptions(
      `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?`,
      `_h5ver=2.0.1&action=share_code_info&share_type=task_50_1111&share_code=${tasksharecode}`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
    ...
    $.get(url, async (err, resp, data) => {
      try {
        if (err) {
          ...
          const findMyCode = sharecodeArr.findIndex(
            (i) => i.indexOf(tasksharecode1) > -1
          );
